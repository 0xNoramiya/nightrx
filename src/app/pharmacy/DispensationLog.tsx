  ...
  if (dispensations.length === 0) {
    return (
      <div className="card">
        ...
        <p className="text-gray-500 text-sm">No dispensations yet.</p>
      </div>
    );
  }
        ...
        {dispensations.map((record, i) => (
          <div key={i} className="flex items-center justify-between py-3 px-4 bg-gray-50 rounded-xl">
            <div>
              ...
              <p className="text-xs text-gray-500">{new Date(record.verifiedAt).toLocaleString()}</p>
            </div>
            <div className="text-emerald-600 text-xs font-medium">Dispensed</div>
          </div>
        ))}
      </div>
    </div>
